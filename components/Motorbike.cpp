...
unsigned int yearMade, double consumptionPoint, string city, bool isAvailable, double minRenterRating, double ratingScore)
: motorId(motorId), ownerId(ownerId), model(model), color(color), engineSize(engineSize), transmissionMode(transmissionMode), 
description(description), yearMade(yearMade), consumptionPoint(consumptionPoint), city(city), isAvailable(isAvailable), 
...
string Motorbike::toString(){
    ...
    ',' + to_string(yearMade) + ',' + to_string(consumptionPoint) + ',' + city + ',' + to_string(isAvailable) + ','   // 1 is true, 0 false
...
void Motorbike::showInfoDetail(){
    ...
    cout << "Motorbike ID: " << motorId << "\tOwner ID: " << ownerId << endl;
    ...
    cout << "Rating Score: " << ratingScore << "\tAvailabe: " << (isAvailable ? "Yes":"No") << endl;
    cout << endl;
}
...
Motorbike Motorbike::createObject(string line){
    ...
    year, consumptionPoint, city, isAvailable, minRenterRating, ratingScore);
}
